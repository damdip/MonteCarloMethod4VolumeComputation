    ...
    // Esempio: Ipersfera k-dimensionale centrata nell'origine
    ...
    int main(int argc , char** argv) {
        ...
        if( strcmp(figura, "sfera") == 0 && argv[4] != NULL ){
            ...
            volume_stimato = monte_carlo_volume(bounds, dim, samples, 
                                                hypersphere_wrapper, &raggio);

            printStats(volume_stimato , volume_teorico);            
        }
        else if(strcmp(figura, "ellisse") == 0 && argv[4] != NULL ){
            double asse_max;
            //acquisizione lunghezze semi-assi
            ...
            for ( int i= 0 ; i < dim ; i++ ){
                ...
                //mantengo il più lungo fra gli assi in modo da inizializzare l'ipercubo (inizializzazione naive)
                if ( array_of_semi_axes[i] > asse_max){ asse_max = array_of_semi_axes[i];}
                }
                else{
            ...
            volume_stimato = monte_carlo_volume(bounds, dim, samples, 
                                                ellipsoid_wrapper, ellissoide);
            
            printStats(volume_stimato , volume_teorico);

            //libero la memoria
            free(array_of_semi_axes);
            free(ellissoide->semi_axes);
            free(ellissoide);
        }
        else{
