...
double monte_carlo_volume(Bounds *bounds, int k, long long int n_samples, 
    ...
    #pragma omp parallel 
    {
        ...
        unsigned int seed = (unsigned int)(time(NULL) + omp_get_thread_num() * 997 + getpid());
        ...
        // Array condiviso per raccogliere i risultati
        static long* array = NULL;
        #pragma omp single
        {
            array = (long*) malloc(sizeof(long) * num_threads);
        }
        ...
        array[thread_id] = my_points_inside;
        
        #pragma omp barrier
        
        // Solo il thread 0 somma i risultati
        #pragma omp single
        {
            for(int i = 0; i < num_threads; i++) {
                sum_of_points_inside += array[i];
            }
            free(array);
        }
